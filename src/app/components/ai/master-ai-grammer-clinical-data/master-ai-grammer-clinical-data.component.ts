...
@Component({
  selector: 'app-master-ai-grammer-clinical-data',
  templateUrl: './master-ai-grammer-clinical-data.component.html',
  styleUrls: ['./master-ai-grammer-clinical-data.component.css']
})
export class MasterAiGrammerClinicalDataComponent implements OnInit {
        ...
        this.EditClinicalDataService.searchAIByID(this.queryParams["aicode"]).subscribe((response: any) => {
          ...
          this.PageTitle.ChangePageTitle(
            this.route.snapshot.paramMap.get("ainame") +
              ...
              ")", 'Master AI - Grammer Review Clinical Data'
          );
  ...
  sendTaskToPublisher() {
    ...
    this.EditClinicalDataService.AddTaskAIToPublisher(dataToSend).subscribe(
      (response: boolean) => {
        if (response){
          this.FinsheMessage = "Data Sent to Publisher"
          this.isFinished = true;
          this.notificationService.success("Data Sent to Publisher Reviewer Successfully");
    ...
    dialogRef.afterClosed().subscribe((data)=>{
      if(data.typeOfClosing == "submit"){
        ...
        const dataToSend={
          ...
          SenderRoleType:"Grammar Reviewer",

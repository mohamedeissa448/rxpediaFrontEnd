...
@Component({
  selector: 'app-master-ai-publish-clinical-data',
  templateUrl: './master-ai-publish-clinical-data.component.html',
  styleUrls: ['./master-ai-publish-clinical-data.component.css']
})
export class MasterAiPublishClinicalDataComponent implements OnInit {
  ...
  ngOnInit() {
    this.PageTitle.ChangePageTitle( this.route.snapshot.paramMap.get("ainame"), 'Master AI - Publish Clinical Data');
        ...
        this.EditClinicalDataService.searchAIByID(this.queryParams["aicode"]).subscribe((response: any) => {
          ...
          this.PageTitle.ChangePageTitle(
            this.route.snapshot.paramMap.get("ainame") +
              ...
              ")"
          ,'Master AI - Publish Clinical Data');
  ...
  sendTaskToPublisher() {
    ...
    this.EditClinicalDataService.PublishAiClinicalData(dataToSend).subscribe(
      (response: boolean) => {
        if (response){
          this.FinsheMessage = "AI Data Published"
          this.isFinished = true;
          this.notificationService.success("AI Published Successfully");
        ...
        this.EditClinicalDataService.ReturnTaskAIToEditor(dataToSend).subscribe(
          (response: boolean) => {
            if (response) {
              ...
              this.notificationService.failed("unFortunately,Data unSent to its Editor!");
              this.actionisLoading = false;
            }
          }
        );
